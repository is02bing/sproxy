...
void hm_init(struct hashmap *map, hashmap_hash_fn hash_fn,
             hashmap_equal_fn equal_fn, void *ctx);
struct hashmap *hm_new(hashmap_hash_fn hash_fn,
                       hashmap_equal_fn equal_fn,
                       void *ctx);
void hm_clear(struct hashmap *map);
void hm_free(struct hashmap *map);

size_t hm_size(const struct hashmap *map);
size_t hm_capacity(const struct hashmap *map);
 ...
 * management.
 */
int hm_insert(struct hashmap *map, long key, long value,
              enum hashmap_insert_strategy strategy,
              long *old_key, long *old_value);

#define hashmap__insert(map, key, value, strategy, old_key, old_value) \
	hm_insert((map), (long)(key), (long)(value), (strategy),  \
